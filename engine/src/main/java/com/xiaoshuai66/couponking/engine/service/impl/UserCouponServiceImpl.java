...
import com.xiaoshuai66.couponking.engine.common.enums.RedisStockDecrementErrorEnum;
import com.xiaoshuai66.couponking.engine.common.enums.UserCouponStatusEnum;
import com.xiaoshuai66.couponking.engine.dao.entity.CouponSettlementDO;
import com.xiaoshuai66.couponking.engine.dao.entity.UserCouponDO;
import com.xiaoshuai66.couponking.engine.dao.mapper.CouponSettlementMapper;
...
import com.xiaoshuai66.couponking.engine.dto.req.*;
...
import static com.xiaoshuai66.couponking.engine.common.constant.EngineRedisConstant.USER_COUPON_TEMPLATE_LIST_KEY;

/**
 * @Description: 用户优惠券业务逻辑实现层
 * @author 赵帅
 * @Create 2025/2/25 14:02
 */
@Slf4j
@Service
...
public class UserCouponServiceImpl implements UserCouponService {
    ...
    public void createPaymentRecord(CouponCreatePaymentReqDTO requestParam) {
        ...
        if (!tryLock) {
            throw new ClientException("正在创建优惠券结算单，请稍后再试");
        }
        ...
        try {
            ...
            JSONObject consumeRule = JSONObject.parseObject(couponTemplate.getReceiveRule());
            ...
            if (couponTemplate.getTarget().equals(0)) {
                ...
                BigDecimal termsOfUse = consumeRule.getBigDecimal("termOfUse");
                ...
                switch (couponTemplate.getType()) {
                    ...
                    case 2: // 折扣券
                        ...
                        if (discountRate.compareTo(maximumDiscountAmount) >= 0) {
                            discountAmount = maximumDiscountAmount;
                        }
                        break;
                    default:
    ...
    public void processPayment(CouponProcessPaymentReqDTO requestParam) {
        ...
        boolean tryLock = lock.tryLock();
        if (!tryLock) {
            throw new ClientException("正在核销优惠券结算单，请稍后再试");
        }
        ...
        transactionTemplate.executeWithoutResult(status -> {
            try {
                ...
                CouponSettlementDO couponSettlementDO = CouponSettlementDO.builder()
                        .status(2)
                        .build();
                int couponSettlementUpdate = couponSettlementMapper.update(couponSettlementDO, couponSettlementUpdateWrapper);
                if (!SqlHelper.retBool(couponSettlementUpdate)) {
                ...
                if (!SqlHelper.retBool(userCouponUpdated)) {
                    ...
                    throw new ServiceException("修改用户优惠券记录异常状态");
                }
            } catch (Exception ex) {
    ...
    public void processRefund(CouponProcessRefundReqDTO requestParam) {
        ...
        boolean tryLock = lock.tryLock();
        if (!tryLock) {
            throw new ClientException("正在执行优惠券退款，请稍后再试");
        }
